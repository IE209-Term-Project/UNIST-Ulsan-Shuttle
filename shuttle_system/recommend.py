...
def slot_status(store, direction, train_time, travel_date, fare=POLICY_FARE):
    """예약 없이 현재 상태(예약 인원 + 배차 셔틀)를 조회."""
    wd = weekday_of(travel_date)
    n_star = breakeven_N(fare)
    count = store.count(direction, train_time, travel_date)
    slot = find_shuttle_slot(direction, train_time, wd, reservations=count, fare=fare)
...
def recommend(store, name, direction, train_time, travel_date, fare=POLICY_FARE, do_book=True, email=''):
    ...
    wd = weekday_of(travel_date)
    n_star = breakeven_N(fare)
    before = store.count(direction, train_time, travel_date)
    slot = find_shuttle_slot(direction, train_time, wd, reservations=before, fare=fare)
    service = slot['service']
    dir_kr = '울산역행' if direction == 'to_station' else '캠퍼스행'

    # 마감 여부·정원·시간 맥락 체크
    shuttle_t = slot.get('shuttle_time') or train_time
    ...
    if service in ('fixed', 'conditional') and do_book and not is_closed and not is_full:
        store.add(name, direction, train_time, travel_date, email=email)
        booked = True
    count = store.count(direction, train_time, travel_date)
    ...
    if service in ('fixed', 'conditional') and (is_closed or is_full):
        ...
        share = taxi_share_logic(store, direction, train_time, travel_date)
        return {'mode': 'alt', 'booked': False, 'reservations': count, 'required': n_star,
                'service': service, 'shuttle_time': slot.get('shuttle_time'),
                'phase': phase,
                'message': (f"❌ 예약 불가 — {reason} 예약이 마감되었습니다. "
                            f"같은 시각 학생 {share['group_size']}명이 함께 택시 카풀 (본인 포함, "
                            f"1인 약 {share['per_person_krw']:,}원) 또는 513 버스를 이용하세요.")}
    ...
    if service == 'fixed':
        ...
        return {'mode': 'shuttle', 'booked': booked, 'reservations': count, 'required': None,
                ...
                'message': (f"✅ {nm}님의 예약이 확정되었습니다.\n"
                            f"{train_time} {dir_kr} 고정 셔틀이 {slot['shuttle_time']}에 출발합니다.\n"
                            f"시간 맞춰 정류장으로 와주세요.")}
    ...
    if service == 'conditional':
        ...
        nm = (name or '학생').strip()
        if count >= n_star:
            msg = (f"🟢 {nm}님의 예약이 확정되었습니다.\n"
                   f"현재 {count}/{n_star}명이 모여 운행 기준을 충족했습니다.{soon}\n\n"
                   f"📨 확정 안내 이메일이 탑승자 전원에게 발송되었습니다. "
                   f"시간 맞춰 정류장으로 와주세요.")
        else:
            need = n_star - count
    ...
    # 셔틀 슬롯 없음
    if time_context(train_time, travel_date) == 'past':
        msg = "⏰ 이미 지난 시각이에요. 다음 KTX/SRT 시각을 선택해 주세요."
    else:
        share = taxi_share_logic(store, direction, train_time, travel_date)
        if share['group_size'] >= 2:
            msg = (f"이 시각엔 배차된 셔틀이 없어요. 같은 시각 학생 {share['group_size']}명이 함께 "
                   f"택시 카풀 시 1인 약 {share['per_person_krw']:,}원 (본인 포함)! 아래 '카풀 신청'을 누르세요. "
