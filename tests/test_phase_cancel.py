...
def test_slot_phase_open_far_future():
    date, hh = _today_at('13:39', days_ahead=2)
...
def test_remove_one_only_my_record():
    ...
    s.add('B', 'to_station', '13:56', '2026-06-04')
    s.add('A', 'to_station', '17:52', '2026-06-04')
    assert s.remove_one('A', 'to_station', '13:56', '2026-06-04') is True
    assert s.count('to_station', '13:56', '2026-06-04') == 1
    assert s.names('to_station', '13:56', '2026-06-04') == ['B']
    # A의 다른 슬롯 예약은 유지
    assert s.count('to_station', '17:52', '2026-06-04') == 1


def test_remove_one_not_found():
    s = MemoryReservationStore()
    s.add('A', 'to_station', '13:56', '2026-06-04')
    assert s.remove_one('Z', 'to_station', '13:56', '2026-06-04') is False
